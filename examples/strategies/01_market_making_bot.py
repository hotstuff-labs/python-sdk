"""Simple market making strategy using short-term trade volume imbalance."""
...
SYMBOL = "BTC-PERP"
RUNTIME_SECONDS = 90
REQUOTE_INTERVAL_MS = 5_000
SIGNAL_WINDOW_MS = 20_000

BASE_ORDER_SIZE = 0.002
MAX_ORDER_SIZE = 0.01
MIN_SIGNAL_VOLUME = 0.03
...
def parse_bbo(detail: Any) -> Optional[BboState]:
    ...
    return BboState(bid=bid, ask=ask)
...
def build_volume_signal(trades: List[TradePoint]) -> VolumeSignal:
    buy_volume = 0.0
    sell_volume = 0.0
    for trade in trades:
        if trade.side == "b":
    ...
    if total < MIN_SIGNAL_VOLUME:
        return VolumeSignal(imbalance=0.0, total=total)

    imbalance = (buy_volume - sell_volume) / total
    return VolumeSignal(imbalance=max(-1.0, min(1.0, imbalance)), total=total)
...
def compute_order_size(signal: VolumeSignal) -> float:
    scale = 1 + min(signal.total / MIN_SIGNAL_VOLUME, 4)
    return min(MAX_ORDER_SIZE, BASE_ORDER_SIZE * scale)


def build_quote_orders(
    instrument_id: int, tick_size: float, bbo: BboState, signal: VolumeSignal
) -> Dict[str, Any]:
    mid = (bbo.bid + bbo.ask) / 2
    skew_bps = signal.imbalance * MAX_SKEW_BPS

    bid_distance_bps = max(1, BASE_HALF_SPREAD_BPS - skew_bps)
    ask_distance_bps = max(1, BASE_HALF_SPREAD_BPS + skew_bps)

    bid_price = round_to_step(mid * (1 - bid_distance_bps / 10_000), tick_size, "down")
    ask_price = round_to_step(mid * (1 + ask_distance_bps / 10_000), tick_size, "up")

    if ask_price <= bid_price:
        ask_price = round(bid_price + tick_size, decimals_from_step(tick_size))

    size = compute_order_size(signal)
    price_decimals = decimals_from_step(tick_size)
    size_text = f"{size:.{SIZE_DECIMALS}f}"
    cloid_base = f"mm-{int(time.time() * 1000)}"
...
def main() -> None:
    print("--------------------------------\nSimple market making (volume signal)\n")
    ...
    try:
        ...
        while int(time.time() * 1000) - started_at < RUNTIME_SECONDS * 1000:
            ...
            prune_trades(recent_trades)
            signal = build_volume_signal(recent_trades)
            ...
            try:
                ...
                print(
                    ...
                    f"signal={signal.imbalance:.2f} "
                    f"volume={signal.total:.4f}"
                )
            except Exception as error:
        ...
        print("Stopping bot and cleaning up...")
